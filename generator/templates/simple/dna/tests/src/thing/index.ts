...
module.exports = (orchestrator) => {
  orchestrator.registerScenario('Create a thing', async (s, t) => {
    ...
    const [alice_simple] = alice_simple_happ.cells
    const thing = await alice_simple.call('simple', 'create_thing', { uuid: uuidv4(), name: 'Test Client 1', location: 'Australia', parent: 'Things' })
  ...
  orchestrator.registerScenario('Create, then list things', async (s, t) => {
    ...
    const [[alice_simple_happ]] = await alice.installAgentsHapps(installation)
    const [alice_simple] = alice_simple_happ.cells
    const thing1 = await alice_simple.call('simple', 'create_thing', { uuid: uuidv4(), name: 'Test Client 1', location: 'Australia', parent: 'Things' });
    const thing2 = await alice_simple.call('simple', 'create_thing', { uuid: uuidv4(), name: 'Test Client 2', location: 'Australia', parent: 'Things' });
    console.log('thing1', thing1)
    console.log('thing2', thing2)
    const thingList = await alice_simple.call('simple', 'list_things', { parent: 'Things' });
  ...
  orchestrator.registerScenario('Create then delete a thing', async (s, t) => {
    ...
    const thing = await alice_simple.call('simple', 'create_thing', { uuid: uuidv4(), name: 'Test Client 1', location: 'Australia', parent: 'Things' });
    console.log('thing', thing)
    const deletedClient = await alice_simple.call('simple', 'delete_thing', thing);
