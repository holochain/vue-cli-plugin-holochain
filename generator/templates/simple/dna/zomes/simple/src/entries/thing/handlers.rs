...
pub(crate) fn delete_thing(thing: Thing) -> SimpleResult<()> {
    ...
    if let Some(Details::Entry(metadata::EntryDetails{headers,..})) = get_details(thing.entry_hash, GetOptions)?{
...
pub(crate) fn list_things(input: ThingListInput) -> SimpleResult<ThingList> {
    ...
    if let Some(thing_link_last) = links.last() {
         if let Some(element) = get(thing_link_last.target.clone(), GetOptions)? {
